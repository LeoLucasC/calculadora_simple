...
// --- NUEVO: Extraer el último proyecto guardado de este usuario ---
...
if ($res_proyectos && $res_proyectos->num_rows > 0) {
    while ($row = $res_proyectos->fetch_assoc()) {
        // Guardamos el JSON usando la URL del fondo como llave
        $proyectos_guardados[$row['nombre_proyecto']] = $row['json_config'];
    }
}

// --- NUEVO: Traer fotos 360 de la empresa del usuario ---
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="xr-panel" id="panel-tools">
            ...
            <div class="section">
                <b>Entorno 360° (Fondo)</b>
                <select id="comboFondo360" onchange="cambiarFondo360(this.value)">
                    <?php if (count($fotos_360) > 0): ?>
                        <?php foreach($fotos_360 as $f): ?>
                            <option value="<?= htmlspecialchars($f['url_archivo']) ?>">
                                <?= htmlspecialchars($f['observaciones'] ?: 'Foto ID: '.$f['id_multimedia']) ?>
                            </option>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <option value="https://aframe.io/aframe/examples/boilerplate/panorama/puydesancy.jpg">Fondo de prueba (Sin fotos subidas)</option>
                    <?php endif; ?>
                </select>
            </div>
    ...
    <script>
        ...
        function cambiarFondo360(url) {
            // 1. Cambiar el fondo visualmente
            let cielo = document.getElementById("cielo360");
            cielo.setAttribute("src", url);

            // 2. Limpiar todos los objetos 3D y textos de la escena actual
            ...
            // 3. Revisar si hay elementos guardados para esta foto específica y cargarlos
        ...
        function guardarEscena() {
            let escenaData = {
                fondo: document.getElementById("cielo360").getAttribute("src"),
                elementos: []
            };
            ...
            let fondoActual = document.getElementById("cielo360").getAttribute("src");
            
            let formData = new FormData();
            formData.append('json_data', jsonFinal);
            formData.append('nombre_proyecto', fondoActual); // Usamos la URL como identificador único
            ...
            .then(data => {
                if(data.status === 'ok') {
                    ...
                    // Guardamos en la memoria local para que no desaparezca si cambiamos de fondo
        ...
        function reconstruirEscena(jsonString) {
            ...
            document.getElementById("cielo360").setAttribute("src", data.fondo);
