...
// Traer todos los marcadores AR del usuario con sus modelos 3D asociados
...
<div id="topbar">
    ...
    <a href="dashboard_cliente.php" class="back-btn">⬅ Volver al Menú</a>
</div>

<?php if(empty($lista_marcadores)): ?>
    <div class="no-data-msg">
        <h2>No hay marcadores</h2>
        <p>Debes crear al menos un marcador en el panel antes de usar el visor AR.</p>
        <br>
        <a href="dashboard_cliente.php" style="color: #00bcd4;">Ir al Dashboard</a>
    </div>
<?php else: ?>
    ...
    <a-scene
       ...
       <?php foreach($lista_marcadores as $index => $m): 
            ...
            // 1. Limpiamos la ruta del audio
            $audio_path = '';
            if (!empty($m['archivo_audio']) && $m['archivo_audio'] != 'NULL') {
                $audio_path = trim($m['archivo_audio'], "'");
            }
        ?>
            <a-marker type="pattern" url="<?= htmlspecialchars($m['archivo_patt']) ?>" emitevents="true" marker-handler="audio: <?= htmlspecialchars($audio_path) ?>">
