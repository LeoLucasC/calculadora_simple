...
// OPCIONAL: Si un ADMIN intenta entrar aquí, lo devolvemos a su panel
...
$res_stats     = $conn->query("SELECT COUNT(*) as total FROM multimedia WHERE id_usuario = $id_user");
$total_subidas = $res_stats->fetch_assoc()['total'];

$actividad_real = $conn->query("
    SELECT m.fecha_hora, m.tipo_archivo, u.nombre as zona 
    FROM multimedia m 
...
<head>
    ...
    <title>KunturVR | Editar Perfil</title>
...
<body>
    ...
    <div class="main-content" id="main-content">
        <div class="page-header">
            <h1>Mi Perfil</h1>
            <p>Gestiona tu información personal y revisa tu actividad en el sistema</p>
        </div>
                ...
                <div class="profile-sidebar">
                    ...
                    <div class="profile-photo">
                        ...
                        <div class="profile-name">
                            ...
                            <p><?= htmlspecialchars($user_data['rol_nombre']) ?></p>
                        </div>

                        <div class="profile-stats">
                            <div class="stat">
                                <div class="stat-value"><?= $total_subidas ?></div>
        ...
        <div class="tab-pane" id="tab-activity">
            <div class="profile-main">
                <h3 class="section-title"><i class="fas fa-history"></i> Historial de Subidas</h3>
                <div class="activity-timeline">
                    <?php if($actividad_real->num_rows > 0): ?>
                        <?php while($act = $actividad_real->fetch_assoc()): ?>
                            <div style="display: flex; gap: 16px; padding: 16px 0; border-bottom: 1px solid #e9eef2;">
                                <div style="width: 40px; height: 40px; background: rgba(2, 54, 117, 0.08); border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                    <i class="fas <?= $act['tipo_archivo'] == 'VIDEO' ? 'fa-video' : 'fa-image' ?>" style="color: #023675;"></i>
                                </div>
                                <div style="flex: 1;">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                                        <strong>Carga de <?= strtolower($act['tipo_archivo']) ?></strong>
                                        <span style="color: #64748b; font-size: 0.85rem;"><?= date('d/m/Y H:i', strtotime($act['fecha_hora'])) ?></span>
                                    </div>
                                    <p style="color: #475569; font-size: 0.9rem;">Ubicación registrada: <?= htmlspecialchars($act['zona']) ?></p>
                                </div>
                            </div>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <p style="text-align:center; color:#64748b; padding:40px;">No se registra actividad reciente.</p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
        ...
        <div class="tab-pane" id="tab-security">
            <div class="profile-main">
                ...
                <form action="" method="POST">
                    ...
                    <div class="form-actions">
                        <button type="submit" class="btn btn-primary"><i class="fas fa-sync-alt"></i> Actualizar Seguridad</button>
                    </div>
                </form>
            </div>
        </div>
